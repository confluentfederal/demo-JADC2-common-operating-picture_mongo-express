#!/usr/bin/env python3
"""JADC2 Mission Readiness Service - GPS-integrated, facility-linked cyber, with action endpoints"""
...
mongo_client, db, atlas_client, atlas_db = None, None, None, None
aircraft_cache, naval_cache, cyber_cache, gps_cache, sigint_cache = {}, {}, {}, {}, {}
action_log = []  # In-memory action audit trail
CYBER_RETENTION_SEC = 600

# US Facilities for reference
US_FACILITIES = [
...
def init_mongo():
    global mongo_client, db, atlas_client, atlas_db
    for i in range(30):
        try:
            ...
            time.sleep(2)
    # Connect to Atlas for enriched data
    try:
        atlas_client = MongoClient(ATLAS_URI, serverSelectionTimeoutMS=10000)
        atlas_client.admin.command('ping')
        atlas_db = atlas_client.jadc2_cop
        logger.info("✓ Atlas MongoDB connected")
    except Exception as e:
        logger.warning(f"Atlas connection failed (will use local): {e}")
        atlas_db = db
    return True
...
def kafka_consumer_thread():
    time.sleep(10)
    try:
        consumer = Consumer({'bootstrap.servers': KAFKA_BOOTSTRAP_SERVERS, 'group.id': 'mission-readiness', 'auto.offset.reset': 'latest'})
        ...
        while True:
            ...
            try:
                ...
                topic = msg.topic()
                if topic == 'ground-force-gps': process_gps(data)
                elif topic == 'military-aircraft': process_aircraft(data)
                elif topic == 'naval-vessels': process_naval(data)
                elif topic == 'cyber-threats': process_cyber(data)
                elif topic == 'sigint-feeds': process_sigint(data)
            except: pass
    except Exception as e:
...
def process_aircraft(data):
    aid = data.get('aircraft_id')
    if aid: aircraft_cache[aid] = {**data, 'last_update': time.time()}

def process_naval(data):
    vid = data.get('vessel_id')
    if vid: naval_cache[vid] = {**data, 'last_update': time.time()}

def process_cyber(data):
    cid = data.get('incident_id')
    if cid: cyber_cache[cid] = {**data, 'last_update': time.time()}

def process_sigint(data):
    sid = data.get('intercept_id')
    if sid: sigint_cache[sid] = {**data, 'last_update': time.time()}
...
def get_aircraft():
    try:
        ...
        if not planes:
            planes = list((atlas_db or db).aircraft_enriched.find({}, {'_id': 0}).sort('timestamp', -1).limit(50))
        for p in planes:
            p.pop('last_update', None)
        return jsonify({'aircraft': planes})
    except Exception as e: return jsonify({'error': str(e)}), 500
...
def get_naval():
    try:
        ...
        if not ships:
            ships = list((atlas_db or db).naval_vessels.find({}, {'_id': 0}).sort('timestamp', -1).limit(50))
        for s in ships:
            s.pop('last_update', None)
        return jsonify({'vessels': ships})
    except Exception as e: return jsonify({'error': str(e)}), 500
...
def get_cyber():
    try:
        now = time.time()
        threats = [v for v in cyber_cache.values() if now - v.get('last_update', 0) < CYBER_RETENTION_SEC]
        if not threats:
            threats = list((atlas_db or db).cyber_enriched.find({}, {'_id': 0}).sort('timestamp', -1).limit(20))
        for t in threats:
            t.pop('last_update', None)
...
def get_sigint():
    try:
        now = time.time()
        intercepts = [v for v in sigint_cache.values() if now - v.get('last_update', 0) < 30]
        if not intercepts:
            intercepts = list((atlas_db or db).sigint_enriched.find({}, {'_id': 0}).sort('timestamp', -1).limit(30))
        for s in intercepts:
            s.pop('last_update', None)
        return jsonify({'intercepts': intercepts})
    except Exception as e: return jsonify({'error': str(e)}), 500

@app.route('/api/alerts')
def get_alerts():
    try:
        alerts = list((atlas_db or db).operational_alerts.find({}, {'_id': 0}).sort('timestamp', -1).limit(25))
        for a in alerts:
            if isinstance(a.get('timestamp'), datetime): a['timestamp'] = a['timestamp'].isoformat()
        return jsonify({'alerts': alerts})
    except Exception as e: return jsonify({'error': str(e)}), 500

# ============================================================
# NEW: ENRICHED DATA ENDPOINTS (from Atlas)
# ============================================================

@app.route('/api/cyber/enriched')
def get_cyber_enriched():
    """Return enriched cyber threats with kill chain phase, risk score"""
    try:
        threats = list((atlas_db or db).cyber_enriched.find({}, {'_id': 0}).sort('timestamp', -1).limit(25))
        for t in threats:
            if isinstance(t.get('timestamp'), datetime): t['timestamp'] = t['timestamp'].isoformat()
        return jsonify({'threats': threats})
    except Exception as e: return jsonify({'error': str(e)}), 500
...
def get_sigint_air_defense():
    """Return SIGINT air defense radar detections"""
    try:
        items = list((atlas_db or db).sigint_air_defense.find({}, {'_id': 0}).sort('timestamp', -1).limit(20))
        return jsonify({'detections': items})
    except Exception as e: return jsonify({'error': str(e)}), 500

@app.route('/api/sigint/jamming')
def get_sigint_jamming():
    """Return SIGINT jamming detections"""
    try:
        items = list((atlas_db or db).sigint_jamming.find({}, {'_id': 0}).sort('timestamp', -1).limit(20))
        return jsonify({'detections': items})
    except Exception as e: return jsonify({'error': str(e)}), 500

@app.route('/api/sigint/hostile-comms')
def get_sigint_hostile_comms():
    """Return hostile communications intercepts"""
    try:
        items = list((atlas_db or db).sigint_hostile_comms.find({}, {'_id': 0}).sort('timestamp', -1).limit(20))
        return jsonify({'intercepts': items})
    except Exception as e: return jsonify({'error': str(e)}), 500

@app.route('/api/embm/strike-candidates')
def get_embm_strikes():
    """Return EMBM targetable jammers"""
    try:
        items = list((atlas_db or db).embm_strike_candidates.find({}, {'_id': 0}).sort('timestamp', -1).limit(20))
        return jsonify({'targets': items})
    except Exception as e: return jsonify({'error': str(e)}), 500

@app.route('/api/embm/active-jammers')
def get_embm_jammers():
    """Return active jammer tracks"""
    try:
        items = list((atlas_db or db).embm_active_jammers.find({}, {'_id': 0}).sort('timestamp', -1).limit(20))
        return jsonify({'jammers': items})
    except Exception as e: return jsonify({'error': str(e)}), 500

@app.route('/api/satellite/passes')
def get_satellite_passes():
    """Return enriched satellite pass data with orbital info"""
    try:
        items = list((atlas_db or db).satellite_pass_enriched.find({}, {'_id': 0}).sort('timestamp', -1).limit(20))
        return jsonify({'passes': items})
    except Exception as e: return jsonify({'error': str(e)}), 500

@app.route('/api/jsir')
def get_jsir():
    """Return JSIR jamming incidents"""
    try:
        items = list((atlas_db or db).jsir_enriched.find({}, {'_id': 0}).sort('timestamp', -1).limit(20))
        return jsonify({'incidents': items})
    except Exception as e: return jsonify({'error': str(e)}), 500

# ============================================================
# NEW: ACTION ENDPOINTS - Cyber Response Actions
# ============================================================

@app.route('/api/cyber/action', methods=['POST'])
def cyber_action():
    """Execute a cyber response action and produce to Kafka for audit trail"""
    try:
        data = request.json
        incident_id = data.get('incident_id')
        action_type = data.get('action')  # block_ip, isolate_system, alert_team, escalate, contain, investigate
        ...
        if action_type == 'block_ip':
            ...
            action_record['details'] = {
                'containment_actions': ['Network segment isolated', 'DNS sinkhole activated', 'IOCs distributed to all endpoints'],
        ...
        # Store in MongoDB
        try:
            (atlas_db or db).cyber_actions.insert_one({**action_record})
        except: pass

        # Produce to Kafka for audit trail
        try:
...
# ============================================================
# NEW: ACTION ENDPOINTS - SIGINT Response Actions
# ============================================================

@app.route('/api/sigint/action', methods=['POST'])
def sigint_action():
    """Execute a SIGINT response action"""
    try:
        data = request.json
        intercept_id = data.get('intercept_id')
        action_type = data.get('action')  # jam, geolocate, alert_ew, task_collection, classify, share_intel
        operator = data.get('operator', 'COP_OPERATOR')
        ...
        if action_type == 'jam':
            action_record['details'] = {
                ...
                'message': f'EA-18G Growler tasked for responsive jamming. ETA to target: 8 minutes.'
            }
        ...
        action_log.append(action_record)
        try:
            (atlas_db or db).sigint_actions.insert_one({**action_record})
        except: pass
        try:
            producer = Producer({'bootstrap.servers': KAFKA_BOOTSTRAP_SERVERS})
...
# ============================================================
# NEW: EMBM Strike Action
# ============================================================

@app.route('/api/embm/action', methods=['POST'])
def embm_action():
    """Execute an EMBM strike/EW response action"""
    try:
        data = request.json
        track_id = data.get('track_id')
        action_type = data.get('action')  # kinetic_strike, ew_attack, refine_geo, monitor
        operator = data.get('operator', 'COP_OPERATOR')
        ...
        action_log.append(action_record)
        try:
            (atlas_db or db).embm_actions.insert_one({**action_record})
        except: pass
